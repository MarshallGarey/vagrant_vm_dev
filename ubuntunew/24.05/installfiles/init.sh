...
function cp_tmp_dir()
{
	...
	cd "${new_bk}"
	# Backup old files
	old_dir="${install_path}/${new}"
	if [ $(ls "${old_dir}" | wc -l) -ne 0 ]
	then
		mv "${old_dir}"/* "${install_path}/${new_bk}/"
	fi
	# Create new files
...
# * vagrant scp the scripts to /opt/slurm/scripts on the destination ctld/dbd
# vagrant scp:
...
# * vagrant scp the config files to /opt/slurm/etc on the destination ctld/dbd
# * On the ctld host:
#   - build Slurm
...
# TODO: enable multiple clusters at some point.
num_clusters=1
